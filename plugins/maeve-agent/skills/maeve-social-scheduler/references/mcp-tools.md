...
Tool availability is gated by the caller's scopes and the session's active toolsets, so a documented tool can be absent from the current session. Three meta-tools manage this:

- `search_mcp_tools`: find the toolsets and tools for a natural-language intent.
- `get_active_mcp_tools`: report the tools active in the current session.
- `activate_mcp_toolsets`: activate allowed toolsets for the current session.

If a tool from the map below is missing, run `search_mcp_tools` for the task and activate the recommended toolset before falling back to the CLI.
...
| `get_content_counts`            | `maeve content:failed-count --workspace <workspaceId>`, `content:pending-approval-count`                     | Read-only failed content count, plus pending approval count when requested on a Standard or Premium plan.                                                                                                                                                     |
| `list_tasks`                    | Public API fallback only.                                                                                    | Read-only task board: columns, active tasks, members, and task details. No CLI command; use the public API when MCP is not connected.                                                                                                                         |
| `get_task`                      | Public API fallback only.                                                                                    | Read-only single task detail, with optional comments and activity. No CLI command; use the public API when MCP is not connected.                                                                                                                              |
| `list_tasks_for_post`           | Public API fallback only.                                                                                    | Read-only tasks linked to one content post. No CLI command; use the public API when MCP is not connected.                                                                                                                                                     |
| `list_archived_tasks`           | Public API fallback only.                                                                                    | Read-only archived task list. No CLI command; use the public API when MCP is not connected.                                                                                                                                                                   |
